...
layout = """
    <h1> Proyecto Web (LP3) | Elias Reyes </h1>
    <hr>
    <ul>
        <li>
...
def saludo(request):
    ...
    return render(request,'saludo.html',{
        'titulo':'Bienvenidos',
        'nombre_autor':'Elias Reyes'})

def rango(request):
    a=10
    b=20
    rango_numero = range(a,b+1)
    return render(request,'rango.html',{
        'titulo':'Rango',
        'a': a,
        'b': b,
        'rango_numeros':rango_numero})
...
def crear_articulo(request,titulo, contenido, publicado):
    ...
    articulo.save()
    return HttpResponse(f"Articulo Creado: {articulo.titulo} - {articulo.contenido} - {articulo.publicado}")
